...
class SandboxExecutor:
    ...
    def _build_test_script(self, issue_data: IssueData, patch: PatchResult) -> str:
        """Build a self-contained Python script that applies and tests the patch."""

        # Clean the patched code — strip any extra indentation
        clean_patch = textwrap.dedent(patch.patched_code).strip()

        # Rename each def to patched_ version
        ...
        # Build script with no leading indentation on any line
        lines = [
            "import pickle",
            "import requests",
            ...
            f'if "Response" in "{patch.class_name}":',
            "    try:",
            ...
            "    except NameError:",
            "        pass",
            f'elif "PreparedRequest" in "{patch.class_name}":',
            "    try:",
            ...
            'assert restored2._next.url == "https://example.com/next", "_next.url wrong!"',
            'print("  PASS")',
            "",
            "# Test 3: _next=None works",
